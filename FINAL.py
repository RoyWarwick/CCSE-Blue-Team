...
# topic assignment
message = subscribe.simple("test", hostname="192.168.1.12") # subscribe to the aggregator topic
...
#publishing to aggregator on the topic received from the aggregator
    ...
    publish.single(topic=topicfinal, payload=s, hostname="192.168.1.12", port=8883, tls={'ca_certs':"/etc/mosquitto/ca_certificates/ca.crt",'certfile':"/etc/mosquitto/certs/server.crt",'keyfile':"/etc/mosquitto/certs/server.key",})
    time.sleep(5)
